...
{
	FNAME=$1
	[ -z "$FNAME" ] && fatal "remove_LaunchDaemon(${FDIR}) must be passed a valid file name"
	# remove the framework directory
	if [ -e /Library/LaunchDaemons/${FNAME} ]; then
		echo "removing LaunchDaemon: $FDIR"
...
{
    FNAME=$1
    [ -z "$FNAME" ] && fatal "remove_LaunchAgent(${FDIR}) must be passed a valid file name"
...
function remove_LaunchAgent()
{
	...
	if [ -e /Library/LaunchAgents/${FNAME} ]; then
		echo "removing LaunchAgent: $FDIR"
...
{
	FNAME=$1
	[ -z "$FNAME" ] && fatal "remove_Preferences(${FDIR}) must be passed a valid file name"
	# remove the preferences file
	if [ -e "/Library/Preferences/${FNAME}" ]; then
		echo "removing Preference: $FDIR"
		rm -rf "/Library/Preferences/${FNAME}"
		[ "$?" != "0" ] && ( echo "ERROR: failed to remove file: ${FNAME}  \n\t$!"; exit 1 )
		((REMOVED++))
	fi
}
...
#	if [ -e '/Library/Application Support/Kaspersky Lab/KAV/Binaries/UninstallScript' ]; then
