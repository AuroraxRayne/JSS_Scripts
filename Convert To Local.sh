...
if [ "$sw_vers_MajorNumber" -ge 13 ]; then
	echo "Major Number is greater or equal to: $sw_vers_MajorNumber"
	/usr/sbin/sysadminctl -adminUser $FVtmpUser -adminPassword $FVtmpUserPASSWORD -deleteUser $USERNAME -keepHome
else
	echo "Major Number is less than: $sw_vers_MajorNumber"
...
if [ "$sw_vers_MajorNumber" -ge 13 ]; then
	echo "Major Number is greater or equal to: $sw_vers_MajorNumber"
	/usr/sbin/sysadminctl -adminUser $FVtmpUser -adminPassword $FVtmpUserPASSWORD -addUser $USERNAME -fullName "$FULLNAME" -password $PASSWORD $MAKE_ADMIN # -UID $LOCALUSERID
else
	echo "Major Number is less than: $sw_vers_MajorNumber"
	/usr/sbin/sysadminctl -addUser $USERNAME -fullName "$FULLNAME" -password $PASSWORD $MAKE_ADMIN # -UID $LOCALUSERID
fi
#/bin/echo "Adding Secure Token to new account"
#/usr/sbin/sysadminctl -adminUser $FVtmpUser -adminPassword $FVtmpUserPASSWORD -secureTokenOn $USERNAME -password $PASSWORD
...
if [[ "$FVtmpUserCreated" == "yes" ]]; then
	...
	</plist>' > /private/tmp/fvenable2.plist
	#
	/usr/bin/fdesetup add -inputplist < /tmp/fvenable2.plist -verbose
	# clean up
	/bin/rm -f /tmp/fvenable2.plist
	# Delete FVtmpUser
	if [ "$sw_vers_MajorNumber" -ge 13 ]; then
		echo "Major Version is greater or equal to: $sw_vers_MajorNumber"
		/usr/sbin/sysadminctl -adminUser $USERNAME -adminPassword $PASSWORD -deleteUser $FVtmpUser
...
jamf policy -trigger disjoinman
sleep 2
jamf manage
sleep 2
jamf recon
...
sleep 2
jamf policy -trigger shutdownnow
