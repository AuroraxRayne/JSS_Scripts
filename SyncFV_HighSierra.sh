...
# Function to decrypt password passed to script
...
echo "checking fdesetup list for $CURRENT_USER removed"
test_FV_USERS2="$(fdesetup list)"
sleep 2
if egrep -q "^${CURRENT_USER}," <<< "$test_FV_USERS2"; then
    echo "[ERROR] $CURRENT_USER is on the list of FileVault enabled users:"
